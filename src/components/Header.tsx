      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-x-0 top-[73px] bottom-0 z-40 overflow-y-auto bg-paper md:hidden"
          >
            <nav className="mx-auto flex w-[min(1240px,92vw)] flex-col pt-6">
              {links.map((l, i) => (
                <Link
                  key={i}
                  href={l.href}
                  onClick={() => setOpen(false)}
                  className="flex items-center gap-4 border-b border-paper-2 py-5 font-display text-[1.7rem]"
                >
                  <Hand color="#E23B8A" size={26} />
                  {l.label}
                </Link>
              ))}
              <a
                href="/#cta"
                onClick={() => setOpen(false)}
                className="mt-8 inline-flex items-center justify-center gap-2 rounded-full bg-magenta px-6 py-4 text-[1.05rem] font-semibold text-white"
              >
                {t("cta")} <span>→</span>
              </a>
            </nav>
